...
function display_news($news) {
  ...
  $html .= '<div class="panel-body">' . $parsedown->text(ReplaceSmilies(nl2br($news['Text']))) . '</div>';  
...
function user_news() {
  ...
  $news = sql_select("SELECT * FROM `News` WHERE `Treffen` = 0 ORDER BY `Datum` DESC LIMIT " . sql_escape($page * $DISPLAY_NEWS) . ", " . sql_escape($DISPLAY_NEWS));
  foreach ($news as $entry)
    $html .= display_news($entry);
